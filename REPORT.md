...
| IP09 | → Prometheus/Grafana | ✅ | `ip09-prometheus-targets.json`: 9/10 target `up` (chỉ `lab28-vllm-optional` down = đúng dự kiến), rule group `lab28-slo` (2 rule); `ip09-grafana-dashboards.json`: dashboard `Lab 28 Platform Overview`; `ip09-alert-delivery.json`: `Lab28ApiUnavailable` đã delivery `firing` và `resolved` qua Alertmanager webhook. |
...
| **Kubernetes/Argo CD chạy thật** | Cluster/apply ✅ / workload ❌ | Cluster `kind-lab28` đã apply namespace, ConfigMap, Deployment, Service, HPA/PDB, NetworkPolicy, Gateway/HTTPRoute và Argo CD Application. Workload chưa healthy vì image `ghcr.io/vinuni-ai20k/day28-platform-api:3.0.0` bị GHCR trả `403` khi pull; vì thế chưa có bằng chứng drift/self-heal/rollback hợp lệ. Cần image registry cho phép pull hoặc image release public trước khi chạy lại. |
...
- [ ] K8s/GitOps workload healthy + drift/self-heal/rollback — cluster `kind` và Argo Application đã apply, nhưng image GHCR bị `403` khi pull
